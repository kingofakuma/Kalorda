...
def visualize_layout(image_path, layout_results, save_path, alpha=0.3):
    ...
    for idx, layout_res in enumerate(layout_results):
        if "bbox" not in layout_res:
            return
        bbox, label, reading_order, tags = (
            layout_res["bbox"],
            layout_res["label"],
            layout_res["reading_order"],
            layout_res["tags"],
        )
        ...
        # Draw text
        cv2.putText(image, label_text, (text_x, text_y), font, font_scale, (0, 0, 0), thickness)
...
def check_bbox_overlap(layout_results_list, image, iou_threshold=0.1, overlap_box_ratio=0.25):
    ...
    if overlap_ratio > overlap_box_ratio:
        print(
            f"⚠️ High overlap detected ({overlap_ratio:.2%} > {overlap_box_ratio:.2%}), treating as distorted/photographed document"
        )
        return True
