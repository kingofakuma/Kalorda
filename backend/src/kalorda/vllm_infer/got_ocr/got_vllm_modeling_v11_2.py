...
from typing import Any, ClassVar, List, Literal, Optional, Protocol, TypedDict, TypeVar, Union
...
from vllm.model_executor.models.utils import PPMissingLayer, maybe_prefix, merge_multimodal_embeddings
...
from vllm.multimodal.parse import ModalityDataItems, MultiModalDataItems, MultiModalDataParser
...
class Qwen2GotModel(Qwen2Model):
    ...
    def forward(
        ...
        hidden_states = super(Qwen2GotModel, self).forward(
            input_ids=None, positions=positions, intermediate_tensors=intermediate_tensors, inputs_embeds=inputs_embeds
        )
    ...
    def get_dummy_mm_data(
        ...
        return {
            "image": self._get_dummy_images(
                width=_IMAGE_SIZE, height=_IMAGE_SIZE, num_images=num_images, overrides=image_overrides
            ),
        }
...
@MULTIMODAL_REGISTRY.register_processor(
    Qwen2GotMultiModalProcessor, info=Qwen2GotProcessingInfo, dummy_inputs=Qwen2GotDummyInputsBuilder
)
class Qwen2GotForCausalLM(Qwen2ForCausalLM, SupportsMultiModal):
    ...
    #     image_input = Qwen2GotImageInputs(type="image_embeds", image_embeds=image_features)
    ...
    def embed_input_ids(
        self, input_ids: torch.Tensor, multimodal_embeddings: Optional[NestedTensors] = None, is_multimodal: bool = True
    ) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(
                ...
                placeholder_token_id=_IMAGE_TOKEN_ID,
            )
