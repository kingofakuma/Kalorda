...
const { layoutConfig, isDarkTheme, setDrakMode } = useLayout();
...
const preset = ref<any>(layoutConfig.preset);
const presetOptions = ref<any>(Object.keys(presets));

const menuMode = ref<any>(layoutConfig.menuMode);
const menuModeOptions = ref<any>([
    { label: t('app.static'), value: 'static' },
    { label: t('app.overlay'), value: 'overlay' }
]);

const primaryColors = ref<any>([
    { name: 'noir', palette: {} },
...
function getPresetExt() {
    const color = primaryColors.value.find((c: any) => c.name === layoutConfig.primary);
...
function updateColors(type: any, color: any) {
    if (type === 'primary') {
...
onMounted(() => {
    ...
    setTimeout(() => {
        const primary = primaryColors.value.filter((s: any) => s.name == layoutConfig.primary)[0];
        const surface = surfaces.value.filter((s: any) => s.name == layoutConfig.surface)[0];
...
<template>
    ...
    >
        <div class="flex flex-col gap-4">
            <div>
                <span class="text-sm text-muted-color font-semibold">{{ t('app.primary') }}</span>
            ...
            <div>
                <span class="text-sm text-muted-color font-semibold">{{ t('app.surface') }}</span>
                <div class="pt-2 flex gap-2 flex-wrap justify-between">
                    <button
            ...
            <div class="flex flex-col gap-2">
                <span class="text-sm text-muted-color font-semibold">{{ t('app.presets') }}</span>
                <SelectButton v-model="preset" @change="onPresetChange" :options="presetOptions" :allowEmpty="false" />
            </div>
            <div class="flex flex-col gap-2">
                <span class="text-sm text-muted-color font-semibold">{{ t('app.menuMode') }}</span>
