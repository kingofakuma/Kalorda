...
function checkActiveRoute(item: any) {
    return route.path.toLowerCase() === item.to.toLowerCase();
}
</script>

<template>
    <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
        <div v-if="root && item.visible !== false" class="layout-menuitem-root-text">{{ t(`route.title.${item.label}`) }}</div>
        <a v-if="(!item.to || item.items) && item.visible !== false" :href="item.url" @click="itemClick($event, item)" :class="item.class" :target="item.target" tabindex="0">
            ...
            <span class="layout-menuitem-text">{{ t(`route.title.${item.label}`) }}</span>
