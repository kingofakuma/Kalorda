...
from vllm.transformers_utils.configs.deepseek_vl2 import DeepseekVLV2Config, MlpProjectorConfig, VisionEncoderConfig
...
class DeepseekOCRDummyInputsBuilder(BaseDummyInputsBuilder[DeepseekOCRProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        # get tokenizer from processing info
        # edit by zhaoying
            ...
            return {
                "image": DeepseekOCRProcessor(tokenizer=tokenizer).tokenize_with_images(
                    images=self._get_dummy_images(
                        width=max_image_size.width, height=max_image_size.height, num_images=num_images
                    ),
                    bos=True,
                    eos=True,
                    cropping=CROP_MODE,
...
class DeepseekOCRMultiModalProcessor(BaseMultiModalProcessor[DeepseekOCRProcessingInfo]):
    ...
    def _call_hf_processor(
        ...
        tok_kwargs: Mapping[str, object],  # edit by zhaoying
    ) -> BatchFeature:
        ...
        if mm_data:
            processed_outputs = self.info.ctx.call_hf_processor(
                ...
                dict(**mm_kwargs, **tok_kwargs),  # edit by zhaoying
            )
    ...
    def _cached_apply_hf_processor(
        ...
        hf_processor_mm_kwargs: Mapping[str, object],
        tokenization_kwargs: Optional[Mapping[str, object]] = None,  # edit by zhaoying
        *,
...
@MULTIMODAL_REGISTRY.register_processor(
    DeepseekOCRMultiModalProcessor, info=DeepseekOCRProcessingInfo, dummy_inputs=DeepseekOCRDummyInputsBuilder
)
    ...
    def _parse_and_validate_image_input(self, **kwargs: object):
        ...
        if pixel_values is not None:
            if not isinstance(pixel_values, (torch.Tensor, list)):
                raise ValueError("Incorrect type of pixel values. " f"Got type: {type(pixel_values)}")

            if not isinstance(images_spatial_crop, (torch.Tensor, list)):
                raise ValueError("Incorrect type of image sizes. " f"Got type: {type(images_spatial_crop)}")

            if not isinstance(images_crop, (torch.Tensor, list)):
                raise ValueError("Incorrect type of image crop. " f"Got type: {type(images_crop)}")
